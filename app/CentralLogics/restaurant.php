...
//     {
//         // Handle null or undefined values for longitude and latitude
...
class RestaurantLogic
{
    public static function get_restaurants($home_restaurant,$big_restaurant,$street_shop,$zone_id, $filter, $limit = 10, $offset = 1, $type='all',$name= null,$longitude=0,$latitude=0,$cuisine='all', $veg = null ,$non_veg = null ,$discount = null,$top_rated = null)
    // public static function get_restaurants($zone_id, $filter, $limit = 10, $offset = 1, $type='all',$name= null,$longitude=0,$latitude=0,$cuisine='all', $veg = null ,$non_veg = null ,$discount = null,$top_rated = null)
    {
        $paginator = Restaurant::
        withOpen($longitude,$latitude)
        ...
        ->when(isset($discount)  && $discount == 1  , function($q) {
            ...
            $q->whereHas('foods',function($query){
                $query->where('discount','>',0)->where('status',1);
        ...
        ->when(isset($big_restaurant) && $big_restaurant == 1 , function($q) {
            $q->where('restaurant_type','Big Restaurant');
        })
        ->when(isset($home_restaurant) && $home_restaurant == 1 , function($q) {
            $q->where('restaurant_type','Home Restaurant');
        })
        ->when(isset($street_shop) && $street_shop == 1 , function($q) {
            $q->where('restaurant_type','Street Shop');
        })
        ->Active()
        ->type($type)
        ->cuisine($cuisine)
    ...
    */
    ...
    /*new get_popular_restaurants >> THis is still testing
    public static function get_popular_restaurants($zone_id, $limit = 10, $offset = 1, $type='all',$longitude=0,$latitude=0 ,$veg = null ,$non_veg = null ,$discount = null,$top_rated = null)
    {
        $startDate = Carbon::now()->startOfMonth();
        $paginator = Restaurant::with(['reviews','discount'=>function($q){//withOpen($longitude,$latitude)
        ...
        //     $query->whereRaw('DATE(created_at) >= ?', [$startDate]);//->whereDate('created_at', '>=', $startDate)//->where('created_at', '<=', $startDate)
