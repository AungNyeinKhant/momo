...
class DeliverymanController extends Controller
{
    ...
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        //=================================Open Below Code if u want comission to fully work(By ANK 25.1.2024)======================

        $orders = Order::with(['customer', 'restaurant'])->whereIn('order_status', ['confirmed','processing','handover'])->whereNull('subscription_id');//This is for comission

        if($dm->type == 'zone_wise')
            {
                /*
                $orders = $orders->whereHas('restaurant', function($qu) use($dm) {
                    $qu->where('zone_id', $dm->zone_id);
                });
                */
                ...
                ->orWhereHas('restaurant', function($q) use($dm){
        ...
        /*=============================ANK Cmt off this and add this to main $orders variable(25.1.2024)=======================below code is for subscription
        */
        if(config('order_confirmation_model') == 'deliveryman' && $dm->type == 'zone_wise')
        {
            ...
            $orders = $orders->whereIn('order_status', ['confirmed','processing','handover'])->whereNull('subscription_id');
        };
            //     ->where(function($query){
            //     $query->where(function($query){
                       
            //             $query->whereIn('order_status', ['confirmed','processing','handover'])->whereNull('subscription_id');
            //              // $query->where('order_status', 'pending')->whereNull('subscription_id');//->whereNotNull('subscription_id')
            //         });
        ...
        if(!empty($orders)){
            $data = [
            'title' => translate('messages.new_order'),
            'description' => translate('messages.new_order_arrived'),
            'order_id' => '',
            'image' => '',
            'type' => 'assign'
            ];
            if(Helpers::send_push_notif_to_device($dm->fcm_token, $data)){
