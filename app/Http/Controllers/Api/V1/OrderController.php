...
class OrderController extends Controller
{
    public function track_order(Request $request)
    {
        ...
        $order = Order::with(['restaurant','restaurant.restaurant_sub', 'refund', 'delivery_man', 'delivery_man.rating','subscription','payments'])->withCount('details')->where(['id' => $request['order_id'], 'user_id' => $request->user()->id])->Notpos()->first();
        if($order){
        ...
        //====================================Distance Alert for Customer by ANK(1.2.2024)==============================
        //$map_api_key = BusinessSetting::where(['key' => 'map_api_key_server'])->first()?->value ?? null;
        // $destination_lat = $order['delivery_address'];//['latitude']
        // $test = data_get($order, 'delivery_address.longitude', null);
        //  $destination_lng = $order['delivery_address']['longitude']; << this show error on server
        // $origin_lat = $order['delivery_man']['lat'];
        // $origin_lng = $order['delivery_man']['lng'];
        // $response = Http::get('https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $origin_lat . ',' . $origin_lng . '&destinations=' . $destination_lat . ',' . $destination_lng . '&key=' . $map_api_key . '&mode=driving');
        // $minSec = $response['rows']['elements']['duration']['value'];
        
        $destination_lat = data_get($order, 'delivery_address.latitude', null);
        $destination_lng = data_get($order, 'delivery_address.longitude', null);
        $origin_lat = data_get($order, 'delivery_man.0.lat', null);
        $origin_lng = data_get($order, 'delivery_man.0.lng', null);
        $map_api_key = BusinessSetting::where(['key' => 'map_api_key_server'])->first()?->value ?? null;
        $googleResponse = Http::get('https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $origin_lat . ',' . $origin_lng . '&destinations=' . $destination_lat . ',' . $destination_lng . '&key=' . $map_api_key . '&mode=driving');
        $mapResponse = $googleResponse->json();
        $minSec = data_get($mapResponse, 'rows.0.elements.0.duration.value', null);
        
        
                
        
        if($minSec <= 300 && $order->order_status == 'picked_up' && $order->customer_checked == 1){
            $data = [
                    'title' => translate('messages.order_alert'),
                    'description' => translate('messages.Your_order_will_be_there_within_5_minute'),
                    'order_id' => $order->id,
                    'image' => '',
                    'type' => 'order_status',
                ];
            if(Helpers::send_push_notif_to_device($order->customer->cm_firebase_token, $data)){
                ...
                DB::update('UPDATE orders SET customer_checked = 0 WHERE id = ?', [$request['order_id']]);
            }
            
            
        }
        
        //=============================Distance Alert for Customer by ANK End ===========================
        ...
        public function kpay_image(Request $request){
        $validator = Validator::make($request->all(), [
            'kpay_image' => 'required',
        ]);
        if ($validator->fails()) {
        ...
        $kapyImg = Helpers::upload(dir: 'onlinepayments/', format: 'png', image: $request->file('kpay_image'));
        

        return response()->json(['kpay_image' => $kapyImg],200);
    }
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'payment_method'=>'required|in:cash_on_delivery,digital_payment,wallet,kpay',
       ...
       if($request['order_type'] == 'delivery' && !$restaurant->free_delivery &&  !isset($delivery_charge) && ($restaurant->restaurant_model == 'subscription' && isset($restaurant->restaurant_sub) && $restaurant->restaurant_sub->self_delivery == 1  || $restaurant->restaurant_model == 'commission' &&  $restaurant->self_delivery_system == 1 )){
        ...
        $order->order_status = ($request->partial_payment ? 'confirmed' : ($request['payment_method'] == 'digital_payment' ? 'pending' : ($request->payment_method == 'wallet' ? 'processing' : 'pending')));//confirmed,pending
...
//===========Kpay Start==========
        if(($order->payment_method == "kpay")){
            $order->kpay_name   =   $request['kpay_name'];
            $order->kpay_transition =   $request['kpay_transition'];
            $order->kpay_image = $request['kpay_image'];
        ...
        $order->confirmed = $request->payment_method == 'wallet' || $request->payment_method == 'kpay' || $request->partial_payment ? now() : null;
    ...
    public function order_notification(Request $request,$order_id){
        ...
        if($order && !in_array($order->payment_method, ['digital_payment', 'partial_payment'])   ){
            Helpers::send_order_notification($order);
        }
