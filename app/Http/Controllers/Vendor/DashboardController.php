...
class DashboardController extends Controller
{
    ...
    public function restaurant_data()
    {
     ...
     $new_pending_order = DB::table('orders')->where(['restaurant_checked' => 0])->where('restaurant_id', $restaurant?->id)->where('order_status','pending');
        ...
        $new_confirmed_order = DB::table('orders')->where(['restaurant_checked' => 0])->where('restaurant_id', $restaurant?->id)->whereIn('order_status',['confirmed', 'accepted'])->whereNotNull('confirmed')->count();
    ...
    if (!$user->restaurant) {
        // Handle the case when the user doesn't have a restaurant assigned
        // You can redirect the user or display an error message
    }

    // Retrieve the associated restaurant ID
    $restaurantId = $user->restaurant->id;

    // Pass the restaurant ID to the view
    return view('dashboard', compact('restaurantId'));
    }
