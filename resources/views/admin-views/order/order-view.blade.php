        ...
        <div class="row g-1" id="printableArea">
                ...
                <div class="card mb-3 mb-lg-5">
                        ...
                        <div class="order-invoice-left">
                            <h1 class="page-header-title mt-2">
                                ...
                                <div class="d-sm-none d-flex flex-wrap ml-auto align-items-center justify-content-end initial-39-2">
                                    @if (!$subscription && !$editing  && count($order?->payments) == 0 &&  in_array($order->order_status, ['pending', 'confirmed', 'processing', 'accepted']) && $order->restaurant)
                            ...
                            @if ($order->schedule_at && ($order->scheduled || $subscription))
                                <span>
                                    ...
                                    <strong class="text-warning">
                                        {{-- {{  Carbon\Carbon::parse($order['schedule_at'])->locale(app()->getLocale())->translatedFormat('d M Y ' . config('timeformat')) }} --}}
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="row justify-content-end mb-3 px-4">
                             @if($order->online_payment_transition && $order->online_payment_image) 
                            <div class="col-md-6 col-lg-5 initial-39-3">
                                <dl class="row text-sm-right">
                                    <dt class="col-6 text-capitalize">{{ translate('name/number') }}:</dt>
                                    <dd class="col-6 text-right">
                                        {{ $order->online_payment_name }} </dd>
                                    <dt class="col-6">{{ translate('messages.transition_no') }}:
                                    </dt>
                                    <dd class="col-6 text-right">
                                        {{ $order->online_payment_transition }}
                                        <hr>
                                    </dd>
                                    <dt class="col-12 text-center">{{ translate('messages.transition_image') }}:</dt>
                                    <div>
                                        <a target="_blank" href="{{ asset('storage/app/public/onlinepayments/' . $order->online_payment_image) }}" >
                                            <img class="img-fluid" src="{{ asset('storage/app/public/onlinepayments/' . $order->online_payment_image) }}" alt="Transition SS">
                                        </a>
                                    </div>
                                </dl>
                            </div>
                            @endif 
                            <div class="col-md-6 col-lg-7 initial-39-3">
                                <dl class="row text-sm-right">
            ...
            <div class="col-lg-4 order-print-area-right">
                ...
                @if (!empty($order->refund))
                    ...
                    <div class="card mb-2">
                        <div class="card-header border-0 d-block text-center pb-0">
                            ...
                            <span>
                                {{-- {{  Carbon\Carbon::parse($order->refund->created_at)->locale(app()->getLocale())->translatedFormat('d M Y ' . config('timeformat')) }} --}}
                ...
                <div class="row g-1">
                                                        ...
                                                        @endif
                                                            ...
                                                            <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                                                <a class="dropdown-item {{ $order['order_status'] == 'pending' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'pending']) }}','{{ translate('Change_status_to_pending_?') }}')"
                                                                    href="javascript:">{{ translate('messages.pending') }}</a>
                                                                <a class="dropdown-item {{ $order['order_status'] == 'confirmed' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'confirmed']) }}','{{ translate('Change_status_to_confirmed_?') }}')"
                                                                    href="javascript:">{{ translate('messages.confirmed') }}</a>

                                                                <a class="dropdown-item {{ $order['order_status'] == 'processing' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'processing']) }}', '{{ translate('Change_status_to_processing_?') }}','{{ translate('Are_you_sure?') }}', '{{ $max_processing_time }}')"
                                                                    href="javascript:">
                                                                    {{ translate('messages.processing') }}</a>

                                                                <a class="dropdown-item {{ $order['order_status'] == 'handover' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'handover']) }}','{{ translate('Change_status_to_handover_?') }}')"
                                                                    href="javascript:">{{ translate('messages.handover') }}</a>
                                                                <a class="dropdown-item {{ $order['order_status'] == 'picked_up' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'picked_up']) }}','{{ translate('Change_status_to_out_for_delivery_?') }}')"
                                                                    href="javascript:">{{ translate('messages.out_for_delivery') }}</a>
                                                                <a class="dropdown-item {{ $order['order_status'] == 'delivered' ? 'active' : '' }}"
                                                                    onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'delivered']) }}','{{ translate('Change_status_to_delivered_(payment_status_will_be_paid_if_not)_?') }}')"
                    ...
                    @endif
                    {{-- @if (($order['order_type'] != 'take_away' && $order->restaurant && !$order->restaurant->self_delivery_system && !$subscription) || $order->subscription_log) --}}
